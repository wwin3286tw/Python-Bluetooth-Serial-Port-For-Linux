...
def main(restart):
 try:
  ...
  if (error_code=='98'):
   ...
   bsl.server().log(R.msg_level.error,R.local_msg.path_not_found_error)
  if (exceptionString=="Cannot access local device: no available bluetoot devices"):
    bsl.server().log(R.msg_level.error,R.local_msg.bt_not_available)
